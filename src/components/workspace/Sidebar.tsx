...
export default function Sidebar({ open }: { open: boolean }) {
  return (
    <aside
      className={`hidden md:flex flex-col gap-[40%] fixed top-14 md:top-20 left-0 h-[calc(100dvh-3.5rem)] md:h-[calc(100dvh-5rem)]
        border-r border-lightBlue/10 bg- transition-all duration-200
        ${open ? 'w-56' : 'w-16'}`}
    >
      <nav className="p-3 flex flex-col gap-6 xl:gap-10">
        {navConfig.map((item) => (
          <NavItem
            key={item.href}
            open={open}
